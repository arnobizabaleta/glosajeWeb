<?php
    ...
    if(!isset($_SESSION['id'])){
        ...
        if(isset($_GET['id'])){
            ...
            $db = new Database();
            $con = $db->conectar();
            $sql = $con->prepare("SELECT codCategoria,nombre_categoria FROM categorias WHERE codCategoria = ?");
                       ...
                       <div class="row">
                            <div class="col mb-4">
                                   ...
                                   <div class="card card-body">
                                       <form action="editCat.php?id=<?php echo  $idCategoria; ?>" method="post">
                                           <div class="form-group mb-4">
                                             <input type="text" name="nombreUp" class="form-control" value="<?php echo  $nombreCat; ?>" required="" autofocus>
                                           ...
                                           <input type="submit" class="btn btn-primary"value="Editar Categoria" name="update">
...
if(isset($_POST['update'])){
    ...
    $sql_update = $con->prepare("UPDATE categorias SET nombre_categoria = ?  WHERE codCategoria = ?");
    $result_update =  $sql_update->execute([$nombreUp,$row['codCategoria']]);
    $row_uptate = $sql_update->fetch(PDO::FETCH_ASSOC);

    if($result_update){
        //echo $result_update;
        $_SESSION['message'] = "Categoria editada exitosamente";
